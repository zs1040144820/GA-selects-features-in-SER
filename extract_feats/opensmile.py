...
def get_feature_opensmile(config, filepath: str) -> list:
    ...
    # Opensmile 配置文件路径
    opensmile_config_path = os.path.join(config.opensmile_path, r'config\is09-13', config.opensmile_config + '.conf')



    # Opensmile 命令
    # 这儿直接给定命令，没用配置文件里的信息，如果要改提取特征指令，要重写这儿
    cmd = 'cd ' + config.opensmile_path + ' && SMILExtract -C ' + opensmile_config_path + ' -I ' + filepath + ' -O ' + single_feat_path + ' -appendarff 0'

    # cmd = 'cd ' + config.opensmile_path + r'&& SMILExtract -C D:\SoftwareData\PyCharmData\Z\Speech-Emotion-Recognition-master\opensmile\config\is09-13\IS10_paraling.conf -I D:\SoftwareData\PyCharmData\Z\Speech-Emotion-Recognition-master\configs\datasets\CASIA\angry\201-angry-liuchanhg.wav -O D:\SoftwareData\PyCharmData\Z\Speech-Emotion-Recognition-master\features\6-category\opensmile_casia\single_feature.csv -appendarff 0'
    print(cmd)
    os.system(cmd)
...
def load_feature(config, train: bool,show_label:bool=False):
    """
    ...
    Returns:
        返回数据，每一行是一个样本，1440个样本，共384个特征，与数据里对应至标准化后
...
def get_data(config, data_path: str, train: bool,label_id:int = -1,append:bool=False) -> Union[Tuple[np.ndarray], np.ndarray]:
    """
    ...
    # 写表头
    if not append or not os.path.exists(feature_path):
        writer = csv.writer(open(feature_path, 'w'), lineterminator="\n")
        first_row = ['label']
    ...
    writer = csv.writer(open(feature_path, 'a+'),lineterminator="\n")
    ...
    if train:
        ...
        if os.path.isdir(data_path):
            for filename in os.listdir(data_path):
                ...
                feature_vector = get_feature_opensmile(config, filepath)
                feature_vector.insert(0, label_id)
                writer.writerow(feature_vector)
        else:
